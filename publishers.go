...
func publishType1(mQueue MessageQueue, sendKey int) {
	for {
		for i := 0; i < 10; i++ {
			mQueue.send(Message{"publisher1", i}, sendKey)
...
func publishType2(mQueue MessageQueue, sendKey int) {
	for {
		mQueue.send(Message{"publisher2", "hello"}, sendKey)
...
func publishType3(mQueue MessageQueue, sendKey int) {
	for {
		for i := 20; i >= 0; i-- {
			mQueue.send(Message{"publisher3", i}, sendKey)
			fmt.Printf("\n\npublisher 1 at time %v the message is:\n%v", time.Now(), i)
		}
